...
class FasttextEmbeddingSequence(TextRepresentation):
    ...
    def _unitary_build(self, texts: SimpleTypeTexts) -> None:
        ...
        if not self._built:
            ...
            if not self._max_vocabulary is None and (1 - (self._min_freq_percentile / 100)) * len(frequencies) > self._max_vocabulary:
            ...
            auxiliar_embs = MaskedEmbedding(
                input_dim=auxiliar_matrix.shape[0], output_dim=auxiliar_matrix.shape[1], weights=[auxiliar_matrix], trainable=True
            )
